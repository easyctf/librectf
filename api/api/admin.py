...
from models import db, Config, Problems, Teams
from schemas import verify_to_schema
...
def admin_setup_init():
    if utils.is_setup_complete():
        raise WebException("Setup has already been complete.")
...
def admin_info():
    ...
    result = {}
    result["start_time"] = settings.get("start_time", "")
    result["end_time"] = settings.get("end_time", "")
    result["team_size"] = settings.get("team_size", "")
    result["stylesheet"] = settings.get("stylesheet", "")
    result["ctfcal_clientid"] = settings.get("ctfcal_clientid", "")
    result["ctfcal_clientsecret"] = settings.get("ctfcal_clientsecret", "")
...
def github_webhook():
    ...
    try:
        ...
        raise WebException(
            "Github is denying access to this repository. Make sure the public key has been installed correctly.")
