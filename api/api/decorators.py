...
def webhook_wrapper(f):
    ...
    def wrapper(*args, **kwds):
        ...
        try:
            ...
            response = 500
            traceback.print_exc()
            web_result = {"success": 0, "message": "Something went wrong! Please notify us about this immediately.",
        ...
        if not ("TESTING" in current_app.config and current_app.config["TESTING"] == True):
            if request.method == "POST":
                try:
                    ...
                    response = make_response(json.dumps({"success": 0, "message": "Token has been tampered with."}),
                                             403, response_header)
